...
class SubjectToBuildingsProcessor:
    def __init__(self):
        ...
        self.output_file = self.data_dir / "buildings_derived.json"
    ...
    def process_to_buildings(self, subject_data: Dict) -> Dict:
            ...
            for course in subject["courses"]:
                for section in course["sections"]:
                    ...
                    if building_name not in buildings:
                        buildings[building_name] = {"rooms": {}, "total_sections": 0}
                    ...
                    section_info = {
                        ...
                        if course["number"].startswith(subject["code"])
                        else f"{subject['code']} {course['number']}",
                        "title": course["title"],
                        "time": section["time"],
                        "days": section["days"],
        ...
        return {"last_updated": datetime.now().isoformat(), "buildings": buildings}
    ...
    def process(self):
        ...
        total_sections = sum(
            b["total_sections"] for b in building_data["buildings"].values()
        )
        print("\nProcessing complete!")
