...
6. **audit_building_metadata.py**

   - Audits hours and coordinates before loading
   - Excludes buildings with incomplete metadata from the current load
   - Emits GitHub Actions warnings and a job-summary table

7. **load_to_postgres.py**

   - Loads the final data into PostgreSQL database
   - Input: `buildings_enriched.json`
   - Creates and populates database tables
   - Runs weekly through [the Course Explorer GitHub Actions workflow](../.github/workflows/course-explorer-weekly.yml), which selects the active term from `academic_calendar.json` in the America/Chicago timezone and supports manual year/term overrides
   - The workflow paces Course Explorer requests and retries transient failures with exponential backoff

8. **tableau_dailyevents_scraper.py**
...
9. **room_details_scraper.py**

   - Scrapes General Assignment classroom capacities from the [Registrar](https://registrar.illinois.edu/faculty-staff/course-catalog-and-scheduling/classroom-capacities/) (`capacity` + `room_type` per room), then crawls Tech Services [Technology Enhanced Classrooms](https://answers.uillinois.edu/illinois/57128) building/room pages for per-room `equipment` and `photo_urls` (use `--skip-answers --no-load` for Registrar-only JSON, `--answers-delay` to tune politeness)
   - Output: `data/room_details.json`; upserts the `room_details` table when `SUPABASE_URL` / `SUPABASE_SECRET_KEY` are set (use `--no-load` for JSON only, `--html-file` to parse a saved page)
   - Maps Registrar/Answers building names to Course Explorer canonical names where they differ (e.g. Siebel) and splits combined rooms like `0027/1025`
   - Runs monthly through [the room details workflow](../.github/workflows/room-details.yml), and can also be started manually from the Actions tab
...
- `.env.local` with `SUPABASE_URL` and `SUPABASE_SECRET_KEY` (needed for `load_to_postgres.py` and the Tableau cache refresh)
...
6) Audit building metadata → excludes incomplete buildings and reports issues
`python3 audit_building_metadata.py`

7) Load to Postgres (reads `buildings_enriched.json`)
