...
8. **tableau_dailyevents_scraper.py**
   ...
   - The repository must define `SUPABASE_URL` and `SUPABASE_SECRET_KEY` as GitHub Actions repository secrets; the key must be allowed to read `rooms` and invoke `update_daily_events` and `refresh_room_availability_cache`
