-- room_availability_cache.busy_times is a canonical tsmultirange produced by
-- range_agg during cache refresh. Read those pre-merged ranges directly rather
-- than rebuilding the same islands from schedule_data on every cache hit.
