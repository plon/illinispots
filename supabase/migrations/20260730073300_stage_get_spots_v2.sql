...
BEGIN
    ...
    activities AS MATERIALIZED (
        -- Build the requested day's activity stream once. The previous
        -- implementation scanned both source tables independently for current,
        -- next, and remaining occupancy.
        SELECT
            cs.building_name,
            cs.room_number,
        ...
        WHERE cs.day_of_week = check_day
          ...
          AND cs.building_name IN (
              SELECT name FROM day_hours_with_status WHERE is_open
          )
        ...
        WHERE DATE(de.start_time AT TIME ZONE 'America/Chicago') = check_date
          AND de.building_name IN (
              SELECT name FROM day_hours_with_status WHERE is_open
          )
    ),
